...
TODO - Start with the general idea (combine a network that can infer the actions just looking at the objects, effects, and verbal descriptions with the inference of the action via the user movements). Them open a subsection to explain the bayesian network, then another for the HMM and another for the fusion of the two.

MAIN CONTRIBUTION - In this paper, we extend~\cite{salvi:2012:smcb} by relaxing the known-action assumption. We estimate the action performed by a human user during a \hr{} collaborative task, by employing statistical inference methods and \acp{HMM}. This provides two advantages: (i)~we can infer the executed action during training; (ii)~at testing time, we can merge the action information obtained from gesture recognition with the information about affordances.
